...
# >>> FRCM 依赖 <<<
from basicsr.models.archs.modules.DCT_Attention import DCT_Attention   # 你已提供
from basicsr.models.archs.modules.odconv import ODConv2d               # 你已提供
from basicsr.models.archs.modules.EMCAM import MSDC 
...
class FRCM(nn.Module):
    """Freq-RF Coupled Mamba Block"""
    def __init__(self, dim, patch_size):
        super().__init__()
        self.dim = dim
        # ----- 多频率分支 -----
        self.dctAtt = DCT_Attention(dim)               # 1D-DCT 通道注意力
        # ----- 多感受野分支 -----
        self.msdc = MSDC(dim, kernel_sizes=[1,3,5,7], dw_parallel=True)  # 返回 list
        self.odconv = ODConv2d(dim, dim, kernel_size=3, groups=dim, kernel_num=4)
        # ----- 门控融合 -----
        self.gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(dim, dim//16, 1), nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        B, C, H, W = x.shape
        # 1) DCT 注意力
        freq = self.dctAtt(x.view(B,C,-1)).view(B,C,H,W)
        # 2) 多尺度 DW
        multi = self.msdc(freq)          # list of 4 tensors
        local = sum(multi) / len(multi)
        # 3) ODConv 二次自适应
        local = self.odconv(local)
        # 4) Dual-Control Gate （参考 EMA 并行 softmax 思想）
        g = self.gate(x)                 # 来自长程 SS2D 的权重
        out = x * g + local * (1 - g)
        return self.norm(out)

##########################################################################
class EVS(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=3, bias=False, LayerNorm_type='WithBias', att=False, idx=3, patch=128,use_deconv=True, use_spectral=True, use_frcm=False):
        ...
        # 替换为：
        self.frcm = FRCM(dim, patch) if use_frcm else \
                   (SpectralGatingNetwork(dim) if use_spectral else nn.Identity())
    ...
    def forward(self, x):
        ...
        x = x + self.frcm(self.ffn(self.norm2(x)))
        return x
    ...
    def __init__(self,
        ...
        for i in range(num_blocks[0]):
            block = EVS(dim=dim, ffn_expansion_factor=ffn_expansion_factor, bias=bias, att=True, idx=i,
                        patch=384,use_deconv=use_deconv, use_spectral=use_spectral,use_frcm=use_frcm)
        ...
        for i in range(num_blocks[1]):
            block = EVS(dim=dim * 2, ffn_expansion_factor=ffn_expansion_factor, bias=bias, att=True, idx=i,
                        patch=192,use_deconv=use_deconv, use_spectral=use_spectral,use_frcm=use_frcm)
        ...
        for i in range(num_blocks[2]):
            block = EVS(dim=dim * 4, ffn_expansion_factor=ffn_expansion_factor, bias=bias, att=True, idx=i,
                        patch=96,use_deconv=use_deconv, use_spectral=use_spectral,use_frcm=use_frcm)
