...
from basicsr.models.archs.modules.DEConv import *          # DEConv.py 里全部类
from basicsr.models.archs.modules.SpectralGating import *  # SpectralGating.py 里全部类

from basicsr.models.archs.modules.DCT_Attention import DCT_Attention   # 频域分支
from basicsr.models.archs.modules.EMCAM     import MSCB               # 多尺度 3×3/5×5/7×7
...
class EDFFN(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias, use_deconv=True):
        ...
        self.dwconv = DEConv(hidden_features * 2) if use_deconv else \
                      nn.Conv2d(hidden_features * 2, hidden_features * 2, 3, 1, 1,
                                groups=hidden_features * 2, bias=bias)
...
class EVS(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=3, bias=False, LayerNorm_type='WithBias',
                 att=False, idx=3, patch=128, use_spectral=True):
        super().__init__()
        self.att       = att
        self.idx       = idx
        self.region    = 8                       # 固定 8×8 划分
        self.tau       = 0.5                     # 重要性阈值

        # --- 1. 新增：区域重要性预测（仅 1×1 conv）
        self.region_importance = nn.Conv2d(dim, 1, 1, bias=False)

        # --- 2. 新增：Lightweight 分支（DCT-Attention + MSCB）
        self.dct_att = DCT_Attention(dim)
        self.mscb = MSCB(in_channels=dim, out_channels=dim, stride=1, kernel_sizes=[3,5,7], expansion_factor=2)

        # --- 3. 原有 SS2D 保留，但只在重要区域跑
        if self.att:
            self.norm1 = LayerNorm(dim)
            self.ss2d = SS2D(d_model=dim, patch=patch)

        self.norm2 = LayerNorm(dim)
        self.ffn = EDFFN(dim, ffn_expansion_factor, bias)
        self.spectral_gate = SpectralGatingNetwork(dim) if use_spectral else nn.Identity()

    # ------------- 内部：pixel_unshuffle / shuffle 封装 -------------
    def _region_split(self, x):
        b, c, h, w = x.shape
        x = x.view(b, c, h//self.region, self.region, w//self.region, self.region)
        x = x.permute(0, 3, 5, 1, 2, 4).contiguous()
        x = x.view(b, self.region*self.region, c, h//self.region, w//self.region)
        return x                             # B, R*R, C, h, w

    def _region_merge(self, x):
        b, rr, c, h, w = x.shape
        x = x.view(b, self.region, self.region, c, h, w)
        x = x.permute(0, 3, 4, 1, 5, 2).contiguous()
        x = x.view(b, c, h*self.region, w*self.region)
        return x
    ...
    def forward(self, x):
        if self.att:
            # —— 1. 预测重要性
            scores = self.region_importance(x)              # B,1,H,W
            scores_region = self._region_split(scores)      # B,R*R,1,h,w
            mask = (scores_region > self.tau).float()       # 0/1 mask

            # —— 2. 划分特征
            x_region = self._region_split(x)                # B,R*R,C,h,w

            # —— 3. 并行双分支
            out_region = torch.zeros_like(x_region)
            for r in range(self.region * self.region):
                feat = x_region[:, r]                       # B,C,h,w
                if mask[:, r, 0, 0, 0] == 1:                # 重要 → SS2D
                    feat = self.norm1(feat)
                    feat = rearrange(feat, 'b c h w -> b h w c')
                    feat = self.ss2d(feat)
                    feat = rearrange(feat, 'b h w c -> b c h w')
                else:                                       # 次要 → Lightweight
                    feat = self.dct_att(feat)
                    feat = self.mscb(feat)
                out_region[:, r] = feat

            # —— 4. 合并回原图
            x = self._region_merge(out_region)

        # —— 5. 原有 FFN + SpectralGate
        x = x + self.spectral_gate(self.ffn(self.norm2(x)))
        # x = x + self.ffn(self.norm2(x))
        return x

# ##########################################################################
# class EVS(nn.Module):
#     def __init__(self, dim, ffn_expansion_factor=3, bias=False, LayerNorm_type='WithBias', att=False, idx=3, patch=128, use_spectral=True):
...
#         x = x + self.spectral_gate(self.ffn(self.norm2(x)))   # 只在残差外插一门控
#         # x = x + self.ffn(self.norm2(x))
    ...
    def __init__(self,
        ...
        for i in range(num_blocks[0]):
            block = EVS(dim=dim, ffn_expansion_factor=ffn_expansion_factor, bias=bias, att=True, idx=i,
                        patch=384,use_spectral=use_spectral)
