...
def parse_options(is_train=True):
    ...
    parser.add_argument(
        ...
        default="options/train/GoPro.yml",
    )
    parser.add_argument(
        "--launcher",
...
# ===== ADD SUBSET WRAPPER CLASS FOR QUICK TESTING =====
class SubsetWrapper:
    """
    A simple wrapper to limit dataset size without modifying the original dataset class.
    This approach avoids changes to other files in the codebase.
    """
    def __init__(self, dataset, max_samples=None, subset_seed=None):
        self.dataset = dataset
        self.original_len = len(dataset)
        
        if max_samples is not None and max_samples < self.original_len:
            if subset_seed is not None:
                # Use fixed seed for reproducibility
                rng = random.Random(subset_seed)
                indices = list(range(self.original_len))
                rng.shuffle(indices)
                self.indices = indices[:max_samples]
            else:
                # Random subset
                self.indices = random.sample(range(self.original_len), max_samples)
            self.indices.sort()  # Sort for better data loading performance
        else:
            self.indices = list(range(self.original_len))
...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt["datasets"].items():
        if phase == "train":
            ...
            # ===== APPLY SUBSET WRAPPER FOR QUICK TESTING =====
            max_samples = dataset_opt.get("max_samples", None)
            subset_seed = dataset_opt.get("subset_seed", None)
            original_size = len(train_set)
            
            if max_samples is not None:
                train_set = SubsetWrapper(train_set, max_samples, subset_seed)
                logger.info(
                    f"[QUICK TEST MODE] Using subset of training data: "
                    f"{len(train_set)}/{original_size} images"
                )
            # ====================================================
            ...
            max_samples = dataset_opt.get("max_samples", None)
            subset_seed = dataset_opt.get("subset_seed", None)
            original_size = len(val_set)
            
            if max_samples is not None:
                val_set = SubsetWrapper(val_set, max_samples, subset_seed)
                logger.info(
                    f"[QUICK TEST MODE] Using subset of validation data: "
                    f"{len(val_set)}/{original_size} images"
                )
            # ====================================================
...
def main():
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            loss_str = ", ".join(
                [f"{k}: {v:.4f}" for k, v in current_log.items() if "loss" in k.lower()]
            )
            lr = (
                model.get_current_learning_rate()[0]
                if isinstance(model.get_current_learning_rate(), list)
                else model.get_current_learning_rate()
            )
            ...
            if opt.get("val") is not None and (
                current_iter % opt["val"]["val_freq"] == 0 or current_iter == 1000
            ):
