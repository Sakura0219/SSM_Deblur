...
class EMCAM(nn.Module):
    def __init__(self, in_channels=64,out_channels=64, kernel_sizes=[1,3,5], expansion_factor=6, dw_parallel=True, add=True, lgag_ks=3, activation='relu'):
        super(EMCAM,self).__init__()
        eucb_ks = 3 # kernel size for eucb

        self.mscb1 = MSCBLayer(in_channels, out_channels, n=1, stride=1, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel, add=add, activation=activation)
