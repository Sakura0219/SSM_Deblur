...
import numpy as np
class DEConv_2_Simple(nn.Module):
    """简化版的细节增强卷积模块 - 更稳定"""
    def __init__(self, dim):
        super(DEConv_2_Simple, self).__init__()
        self.dim = dim
        
        # 使用标准的Conv2d
        self.conv_cd = nn.Conv2d(dim, dim, 3, padding=1, bias=True)
        self.conv_hd = nn.Conv2d(dim, dim, 3, padding=1, bias=True)
        self.conv_vd = nn.Conv2d(dim, dim, 3, padding=1, bias=True)
        self.conv_ad = nn.Conv2d(dim, dim, 3, padding=1, bias=True)
        self.conv_normal = nn.Conv2d(dim, dim, 3, padding=1, bias=True)
        
        # 特征融合
        self.fusion = nn.Conv2d(dim * 5, dim, 1, bias=True)
    ...
    def forward(self, x):
        # 应用不同方向的卷积
        res_cd = self.conv_cd(x)
        res_hd = self.conv_hd(x)
        res_vd = self.conv_vd(x)
        res_ad = self.conv_ad(x)
        res_normal = self.conv_normal(x)
        
        # 拼接所有特征
        concat_features = torch.cat([res_cd, res_hd, res_vd, res_ad, res_normal], dim=1)
        
        # 融合特征
        fused = self.fusion(concat_features)
        
        # 门控机制
        gate_input = torch.cat([x, fused], dim=1)
...
class EVSSM(nn.Module):
    def __init__(self,
        ...
        # ========== 改进1：添加细节增强模块 ==========
        if self.use_deconv:
            self.detail_enhance = DEConv_2_Simple(dim)
        ...
        if self.use_gated_fusion:
            self.fusion2 = GatedFeatureFusion(dim * 2)
        ...
        if self.use_gated_fusion:
            self.fusion1 = GatedFeatureFusion(dim)
    ...
    def forward(self, inp_img):
        ...
        # ========== 应用细节增强 ==========
        if self.use_deconv:
            inp_enc_level1 = self.detail_enhance(inp_enc_level1)
        ...
        # ========== 应用门控融合 ==========
        if self.use_gated_fusion:
            inp_dec_level2 = self.fusion2(out_enc_level2, inp_dec_level2)
        else:
            inp_dec_level2 = inp_dec_level2 + out_enc_level2  # 原始的简单相加
        out_dec_level2 = self.decoder_level2(inp_dec_level2)
        inp_dec_level1 = self.up2_1(out_dec_level2)
        if self.use_gated_fusion:
            inp_dec_level1 = self.fusion1(out_enc_level1, inp_dec_level1)
        else:
            inp_dec_level1 = inp_dec_level1 + out_enc_level1  # 原始的简单相加
